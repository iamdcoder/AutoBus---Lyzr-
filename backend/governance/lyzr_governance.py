...
class LyzrGovernance:
    ...
    def mode(self) -> str:
        ...
        if self.aims_webhook_url:
            parts.append("lyzr_aims_sink")
        if not parts:
    ...
    def _aims_event(self, event: dict[str, Any]) -> dict[str, Any]:
        return {
            "schema_version": "autobus.aims-event.v1",
            "source": "autobus-negotiator",
            "event": event,
        }

    def publish_event(self, event: dict[str, Any]) -> dict[str, Any]:
        envelope = self._aims_event(event)
        if not self.aims_webhook_url:
            with self.outbox_path.open("a", encoding="utf-8") as file:
                file.write(json.dumps(envelope, sort_keys=True) + "\n")
            return {"published": False, "source": "local_aims_outbox", "queued": True, "path": str(self.outbox_path)}

        headers = {"content-type": "application/json", "accept": "application/json"}
        if self.aims_token:
            headers["authorization"] = f"Bearer {self.aims_token}"
        try:
            response = requests.post(self.aims_webhook_url, headers=headers, json=envelope, timeout=self.timeout)
            response.raise_for_status()
            return {"published": True, "source": "lyzr_aims_sink", "status_code": response.status_code}
        except Exception as exc:
            with self.outbox_path.open("a", encoding="utf-8") as file:
                file.write(json.dumps(envelope, sort_keys=True) + "\n")
            return {"published": False, "source": "lyzr_aims_sink", "queued": True, "reason": str(exc)}
